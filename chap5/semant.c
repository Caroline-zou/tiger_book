	...
	switch(a->kind) {
		...
		case(A_callExp): {
			...
			for(tl = x->u.fun.formals, el = a->u.call.args; tl && el; tl = tl->tail, el = el->tail) {
				expty arg_ty = transExp(venv, tenv, el->head);
		...
		case(A_opExp): {
			expty left, right;
			left = transExp(venv, tenv, a->u.op.left);
			right = transExp(venv, tenv, a->u.op.right);
			switch(a->u.op.oper) {
				case(A_ltOp):
				case(A_leOp):
				case(A_gtOp):
				...
				for(efl = a->u.record.fields; efl; efl = efl-> tail){
					for(fl = typ_ty->u.record; fl; fl = fl->tail) {
						...
						if(fl->head->name == efl->head->name) {
							expty exp_ty = transExp(venv, tenv, efl->head->exp);
		...
		case(A_assignExp): {
			expty var_ty = transVar(venv, tenv, a->u.assign.var);
			expty exp_ty = transExp(venv, tenv, a->u.assign.exp);
		...
		case(A_ifExp): {
			...
			test_ty = transExp(venv, tenv, a->u.iff.test);
			if(test_ty.ty->kind != Ty_int)
				EM_error(a->u.iff.test->pos, "illegal condition expression");
			if(a->u.iff.elsee) {
				exp1_ty = transExp(venv, tenv, a->u.iff.then);
				exp2_ty = transExp(venv, tenv, a->u.iff.elsee);
		...
		case(A_whileExp): {
			...
			test_ty = transExp(venv, tenv, a->u.whilee.test);
			if(test_ty.ty->kind != Ty_int) 
				EM_error(a->u.whilee.test->pos, "illegal condition expression");
			++loop_level;
			body_ty = transExp(venv, tenv, a->u.whilee.body);	
			--loop_level;
		...
		case(A_letExp): {
			...
			exp_ty = transExp(venv, tenv, a->u.let.body);
			S_endScope(tenv);
			S_endScope(venv);
			return exp_ty;
		}
		case(A_arrayExp): {
			...
			if(!array_ty || array_ty->kind != Ty_array) {
				...
				// But nil must be constrained by record type.
				// Therefore, Ty_Void() would be better than Ty_Nil();

				return expTy(0, Ty_Void());			
			}
			size_ty = transExp(venv, tenv, a->u.array.size);
			if(size_ty.ty->kind != Ty_int) 
				EM_error(a->pos, "require integer to declare size");	
			init_ty = transExp(venv, tenv, a->u.array.init);
...
static expty transVar(S_table venv, S_table tenv, A_var v) {
	switch(v->kind) {
		...
		case(A_fieldVar): {
			expty var_ty = transVar(venv, tenv, v->u.field.var);
		...
		case(A_subscriptVar): {
			expty var_ty = transVar(venv, tenv, v->u.subscript.var);
...
static void transDec(S_table venv, S_table tenv, A_dec d) {
	switch(d->kind) {
		case(A_varDec): {
			if(d->u.var.typ) {
				expty init_ty = transExp(venv, tenv, d->u.var.init);
				Ty_ty typ_ty = actualTy(S_look(tenv, d->u.var.typ));
				if(init_ty.ty == typ_ty)
					S_enter(venv, d->u.var.var, E_VarEntry(typ_ty));		
				...
				if(init_ty.ty->kind != Ty_nil)
					S_enter(venv, d->u.var.var, E_VarEntry(init_ty.ty));
				else {
		...
		case(A_functionDec): {
			...
			for(A_fundecList fdl = d->u.function; fdl; fdl = fdl->tail) {
				...
				S_enter(venv, fdl->head->name, E_FunEntry(formalTyList, result_ty));	
			}
        ...
        Ty_ty result_ty;
				...
				formalTyList = makeFormalTyList(tenv, fdl->head->params);
				S_beginScope(venv);
				for(fl = fdl->head->params, tl = formalTyList; tl; tl = tl->tail, fl = fl->tail) 
					S_enter(venv, fl->head->name, E_VarEntry(tl->head));
					expty exp_ty= transExp(venv, tenv, fdl->head->body);
        	// test40.tig
          if(exp_ty.ty != actualTy(result_ty))
