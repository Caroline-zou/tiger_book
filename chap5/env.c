...
S_table E_base_venv() {
	...
	result = Ty_Void();
	S_enter(table, S_Symbol("print"), E_FunEntry(formals, result));

	// Eventually I decided to use Ty_Void() to indicate the function 		// has no argument and to avoid the fact that the formals field of 
	...
	result = Ty_Void();
	S_enter(table, S_Symbol("flush"), E_FunEntry(formals, result));
	// getchar() : string
	formals = 0; // no argument
	result = Ty_String();
	S_enter(table, S_Symbol("getchar"), E_FunEntry(formals, result));
	// ord(s : string) : int
	formals = Ty_TyList(Ty_String(), 0);
	result = Ty_Int();
	S_enter(table, S_Symbol("ord"), E_FunEntry(formals, result));
	// chr(i : int) : string
	formals = Ty_TyList(Ty_Int(), 0);
	result = Ty_String();
	S_enter(table, S_Symbol("chr"), E_FunEntry(formals, result));
	// size(s : string) : int
	formals = Ty_TyList(Ty_String(), 0);
	result = Ty_Int();
	S_enter(table, S_Symbol("size"), E_FunEntry(formals, result));
	// substring(s:string, first:int, n:int) : string
	formals = Ty_TyList(Ty_String(), Ty_TyList(Ty_Int(), Ty_TyList(Ty_Int(), 0)));
	result = Ty_String();
	S_enter(table, S_Symbol("substring"), E_FunEntry(formals, result));
	// concat(s1: string, s2: string) : string
	formals = Ty_TyList(Ty_String(), Ty_TyList(Ty_String(), 0));
	result = Ty_String();
	S_enter(table, S_Symbol("concat"), E_FunEntry(formals, result));
	...
	formals = Ty_TyList(Ty_Int(), 0);
	result = Ty_Int();
	S_enter(table, S_Symbol("not"), E_FunEntry(formals, result));
	// exit(i : int)
	formals = Ty_TyList(Ty_Int(), 0);
	result = Ty_Void();
	S_enter(table, S_Symbol("exit"), E_FunEntry(formals, result));
