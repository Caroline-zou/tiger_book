...
{
 // precondition: the kind of 'exp' must be T_CALL
...
{
  if (!stms) 
		// insert jump 'done' in the end of last basic block  
  ...
  if (stms->head->kind == T_JUMP || stms->head->kind == T_CJUMP) {
    ...
    Temp_label lab = stms->head->u.LABEL;
		// insert jump lab in the end of basic block
...
static C_stmListList mkBlocks(T_stmList stms, Temp_label done)
{
  ...
  if (stms->head->kind != T_LABEL) {
		// insert a label in the begining of the basic block
...
static S_table block_env; /* Symbol*(label) -> T_stmList(block) */
...
static void trace(T_stmList list)
{
  ...
  S_enter(block_env, lab->u.LABEL, NULL); // mark the block
  if (s->kind == T_JUMP) {
  ...
  /* we want false label to follow CJUMP */
    ...
    if (false) {
      ...
      //last->tail->tail = T_StmList(T_Label(false), getNext());
			...
			// I think there would be something wrong in the above statement.
			// According to the page 188, the statement 'CJUMP(cond, a, b, t, f)' 
			// would transfer to three statements, which are 'CJUMP(cond, a, b, t, 
			// ff)', 'LABEL(ff)' and 'JUMP(f)'.
			// Obviously, the former statement supplied by authors has forgot to add
			// 'JUMP(f)'.

			last->tail->tail = T_StmList(T_Label(false), 
			                    T_StmList(T_Jump(T_Name(s->u.CJUMP.false),
